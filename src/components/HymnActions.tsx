...
export default function HymnActions({ hymn }: { hymn: IHymn }) {
  ...
  const buildPlainText = useCallback(() => {
    const lines: string[] = [];
    lines.push(`Hymn ${hymn.hymnNumber} – ${hymn.title}`);
    if (hymn.author) lines.push(`by ${hymn.author}`);
    lines.push("");
    ...
    for (const verse of hymn.verses) {
      ...
      if (hymn.chorus.text) {
        lines.push(hymn.chorus.qualifier ?? "Chorus");
        lines.push(hymn.chorus.text);
        lines.push("");
      }
    }
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    const titleLines = doc.splitTextToSize(
      `Hymn ${hymn.hymnNumber} – ${hymn.title}`,
      maxWidth,
    );
    ...
    if (hymn.author) {
      ...
      doc.setFontSize(11);
      doc.text(`by ${hymn.author}`, margin, y);
      y += 7;
    }
    ...
    for (const verse of hymn.verses) {
      ...
      if (hymn.chorus.text) {
        doc.setFont("helvetica", "bolditalic");
        doc.setFontSize(10);
        const chorusLabel = hymn.chorus.qualifier ?? "Chorus";
        checkPageBreak(6);
        ...
        doc.setFont("helvetica", "italic");
        doc.setFontSize(11);
        const chorusLines = doc.splitTextToSize(hymn.chorus.text, maxWidth - 5);
    ...
    // Tunes / Sol-fa notes
    if (hymn.tunes.length > 0) {
      y += 4;
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      checkPageBreak(8);
      doc.text(`Tune${hymn.tunes.length > 1 ? "s" : ""} / Sol-fa`, margin, y);
      y += 6;

      doc.setFont("helvetica", "normal");
      doc.setFontSize(11);
      for (const tune of hymn.tunes) {
    ...
    doc.save(
      `hymn-${hymn.hymnNumber}-${hymn.title.replace(/\s+/g, "-").toLowerCase()}.pdf`,
    );
  }, [hymn]);
